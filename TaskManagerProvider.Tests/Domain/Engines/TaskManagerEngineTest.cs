...
public class TaskManagerEngineTest : IDisposable
{
    ...
    [Fact]
    public async TTask TaskManagerEngine_FailToDeleteUserAndRestoreAllRelatedTasks()
    {
        const int testUserId = 1;
        ...
        _taskRepositoryMock.Setup(x => x.GetUserTasks(It.Is<int>(t => t == testUserId))).Returns(TTask.FromResult(relatedTasks.AsEnumerable()));
        _taskRepositoryMock.Setup(x => x.DeleteTasks(It.IsAny<IEnumerable<Task>>())).Returns(TTask.FromResult(relatedTasks.Select(t => true).ToArray()));
        _taskRepositoryMock.Setup(x => x.CreateTask(It.IsAny<Task>())).Returns((Task task) => TTask.FromResult(task));
        _userRepositoryMock.Setup(x => x.DeleteUser(It.Is<int>(id => id == testUserId))).Returns(TTask.FromResult(false));

        var result = await _taskManagerEngine.DeleteUser(testUserId);

        Assert.False(result, "The user with the specified Id and all related tasks should be successfully restored.");
        _taskRepositoryMock.Verify(x => x.GetUserTasks(It.Is<int>(t => t == testUserId)), Times.Once());
        _taskRepositoryMock.Verify(x => x.DeleteTasks(It.IsAny<IEnumerable<Task>>()), Times.Once());
        _taskRepositoryMock.Verify(x => x.CreateTask(It.IsAny<Task>()), Times.Exactly(relatedTasksCount));
        _userRepositoryMock.Verify(x => x.DeleteUser(It.Is<int>(id => id == testUserId)), Times.Once);
        ...
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((o, t) => string.Equals($"All related tasks were restored for user with the specifed user Id: {testUserId}", o.ToString(), StringComparison.InvariantCultureIgnoreCase)),
                It.IsAny<Exception>(),
                (Func<It.IsAnyType, Exception?, string>)It.IsAny<object>()),
            Times.Once);
    }
